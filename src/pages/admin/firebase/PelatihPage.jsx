...
const paketPTOptions = [
  { value: "trial", label: "1x Trial", total: 1 },
  { value: "8x", label: "8x Coaching", total: 8 },
  { value: "12x", label: "12x Coaching", total: 12 },
  { value: "16x", label: "16x Coaching", total: 16 },
];
...
// Normalisasi nama: dipakai sebagai KEY pengelompokan (case-insensitive, spasi rapi)
const normKey = (s) => (s || "").trim().toLowerCase().replace(/\s+/g, " ");
// Versi tampilan yang rapi: Title Case, dipakai untuk ditampilkan di UI & Excel
...
export default function PelatihPage() {
  ...
  // Perpanjang dari Data Keseluruhan
  const [perpanjangTarget, setPerpanjangTarget] = useState(null); // { pelatih, member }
  const [perpanjangTargetPaket, setPerpanjangTargetPaket] = useState("8x");
  const [perpanjangTargetLoading, setPerpanjangTargetLoading] = useState(false);

  const toggleExpandPelatih = (p) => {
    setExpandedPelatih(prev => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [data, pend] = await Promise.all([
        api.get("/pelatih"),
        api.get("/pelatih/pending"),
      ]);
      setSesi(data);
      setPendingSesi(pend);
    } catch (err) {
      alert("Gagal memuat data sesi latihan: " + err.message);
    }
    setLoading(false);
  };
  ...
  const cekSisa = async (pelatih, member) => {
    ...
    setCekLoading(true);
    try {
      const data = await api.get(`/pelatih/cek-sisa?pelatih=${encodeURIComponent(pelatih)}&member=${encodeURIComponent(member.trim())}`);
      setSisaInfo(data);
  ...
  const handleSesiSubmit = async (e) => {
    e.preventDefault();
    // Kalau paket habis, paksa tampil modal perpanjang dulu
    if (sisaInfo?.habis) { setShowPerpanjangModal(true); return; }
    try {
      await api.post("/pelatih", sesiForm);
      // Re-check setelah save — kalau sesi yang baru aja dicatat bikin paket jadi penuh, langsung popup
      ...
      if (updatedSisa?.habis) {
        // Tutup form sesi tapi langsung munculin modal "paket baru aja habis"
        setShowSesiForm(false);
        setSesiForm(emptySesiForm);
        setJustHabis(true);
      ...
      alert("Gagal menyimpan sesi latihan: " + err.message);
    }
  };
  ...
  const handlePerpanjang = async () => {
    ...
    try {
      ...
      await cekSisa(sesiForm.pelatih, sesiForm.member);
      fetchData();
    } catch (err) {
      alert("Gagal perpanjang paket: " + err.message);
    }
  ...
  // Group sesi by pelatih -> member (key dinormalisasi case-insensitive, displayName tetap rapi)
  const grouped = {};
  ...
  const toggleSelectAll = () => {
    setSelected(prev => prev.size === sesiFiltered.length ? new Set() : new Set(sesiFiltered.map(s => s.id)));
  };
  ...
  const handleExport = () => {
    if (activeTab === "sesi") {
      ...
      const rows = source.map(s => ({
        "Tanggal": s.tanggal,
        "Pelatih": s.pelatih,
        "Member": toTitleCase(s.member),
        "Paket": paketPTOptions.find(o => o.value === s.paket)?.label || s.paket || "-",
        "Catatan": s.catatan || "",
      }));
      exportToExcel(rows, "Sesi_Latihan", "Sesi");
    } else {
      const rows = [];
      pelatihList.forEach(p => {
        Object.values(grouped[p] || {}).forEach(({ displayName, entries: rawEntries }) => {
          ...
          let cycleStart = 0;
          let currentPaket = entries[entries.length - 1]?.paket || "8x";
          for (let i = entries.length - 1; i >= 0; i--) {
            if (entries[i].placeholder) { cycleStart = i; currentPaket = entries[i].paket || "8x"; break; }
          }
          const cycleEntries = entries.slice(cycleStart);
          const paketInfo = paketPTOptions.find(o => o.value === currentPaket) || paketPTOptions[1];
          const slice = cycleEntries.filter(e => !e.placeholder);
          rows.push({
            "Pelatih": p,
            "Member": displayName,
            "Paket": paketInfo.label,
            "Sesi Selesai": slice.length,
            "Total Sesi": paketInfo.total,
            "Tanggal Terakhir": slice[slice.length - 1]?.tanggal || "-",
          });
        });
      });
  ...
  return (
    <div>
        ...
        <div>
                  ...
                  <div key={p.id} style={{ background: "#fff", border: "1px solid #ffe082", padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={async () => { try { await api.post(`/pelatih/pending/${p.id}/approve`); fetchData(); } catch (err) { alert(err.message); } }}
                        style={{ background: "#2e7d32", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.75rem", padding: "6px 14px", cursor: "pointer" }}>
                        ✓ Setujui
                      </button>
                      <button onClick={async () => { const alasan = prompt("Alasan penolakan (opsional):") || ""; try { await api.post(`/pelatih/pending/${p.id}/reject`, { alasan }); fetchData(); } catch (err) { alert(err.message); } }}
                        style={{ background: "#c62828", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.75rem", padding: "6px 14px", cursor: "pointer" }}>
                        ✕ Tolak
                      </button>
                    </div>
                  </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button onClick={() => setShowSesiForm(true)} style={addBtnStyle}>
                + Catat Sesi Latihan
              </button>
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {pelatihList.map(p => {
              ...
              const memberKeys = Object.keys(members).filter(k =>
                !searchKeseluruhan || normKey(members[k].displayName).includes(normKey(searchKeseluruhan))
              );
              ...
              return (
                <div key={p} style={{ background: "#fff", border: "1px solid #e0e0e0" }}>
                  <button onClick={() => toggleExpandPelatih(p)} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: "#999", background: "#f5f5f5", padding: "2px 8px" }}>
                        {memberKeys.length} member
                      </span>
                    </div>
                      ...
                      {memberKeys.length === 0 ? (
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
                          {memberKeys.map(key => {
                            ...
                            // Pakai createdAt buat sort (konsisten dengan portal & backend)
                            const entries = [...rawEntries].sort((a, b) => {
                              const ta = a.createdAt?._seconds ?? (typeof a.createdAt === "string" ? new Date(a.createdAt).getTime() / 1000 : 0);
                              const tb = b.createdAt?._seconds ?? (typeof b.createdAt === "string" ? new Date(b.createdAt).getTime() / 1000 : 0);
                              return ta - tb;
                            });
                            // Cari cycle terakhir dari placeholder — sama persis dengan logika backend portal
                            let cycleStart = 0;
                            let currentPaket = entries[entries.length - 1]?.paket || "8x";
                            for (let i = entries.length - 1; i >= 0; i--) {
                              if (entries[i].placeholder) { cycleStart = i; currentPaket = entries[i].paket || "8x"; break; }
                            ...
                            const cycleEntries = entries.slice(cycleStart);
                            const paketInfo = paketPTOptions.find(o => o.value === currentPaket) || paketPTOptions[1];
                            const total = paketInfo.total;
                            const slice = cycleEntries.filter(e => !e.placeholder);
                            return (
                              <div key={key} style={{ background: slice.length >= total ? "#fff5f5" : "#fafafa", border: `1px solid ${slice.length >= total ? "#ffcdd2" : "#e0e0e0"}`, padding: 12 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 6 }}>
                                  ...
                                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                    ...
                                    {slice.length >= total && (
                                      <button onClick={() => { setPerpanjangTarget({ pelatih: p, member: displayName }); setPerpanjangTargetPaket("8x"); }}
                                        style={{ background: "#1565c0", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.72rem", padding: "4px 12px", cursor: "pointer" }}>
                                        + Perpanjang
                                      </button>
                                    )}
                                  </div>
                                ...
                                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                                  {Array.from({ length: total }).map((_, i) => {
                                    ...
                                    const milestones = { trial: [1], "8x": [4, 8], "12x": [6, 12], "16x": [8, 16] };
                                    const msList = milestones[paketInfo.value || currentPaket] || [];
                                    const isMilestone = msList.includes(i + 1);
                                    return (
                                      <div key={i} style={{ position: "relative", width: 30, height: 30 }}>
                                        <div style={{ width: 30, height: 30, background: done ? "#e8f5e9" : "#fff", border: `${isMilestone ? "2px" : "1px"} solid ${done ? "#a5d6a7" : isMilestone ? "#f57f17" : "#ddd"}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                                          <span style={{ color: done ? "#2e7d32" : isMilestone ? "#f57f17" : "#999", fontWeight: 700, fontSize: "0.65rem" }}>{done ? "✓" : i + 1}</span>
                                          {done && <span style={{ color: "#2e7d32", fontSize: "0.42rem", fontFamily: "var(--font-mono)" }}>{formatTgl(entry.tanggal)}</span>}
                                        </div>
                                        {isMilestone && (
                                          <div style={{ position: "absolute", top: -6, right: -4, background: "#f57f17", color: "#fff", fontSize: "0.38rem", fontFamily: "var(--font-mono)", fontWeight: 700, padding: "1px 3px", borderRadius: 2 }}>Rp</div>
                                        )}
                                      </div>
                                    );
                                  })}
                                </div>
                                <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: slice.length >= total ? "#c62828" : "#888", marginTop: 6, fontWeight: slice.length >= total ? 700 : 400 }}>
                                  {slice.length} / {total} sesi {slice.length >= total ? "— HABIS" : "selesai"}
                                </div>
                              </div>
                            );
                          })}
          ...
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", width: "100%", maxWidth: 440, padding: 28 }}>
            ...
            <form onSubmit={handleSesiSubmit}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                />
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", color: "#aaa", marginTop: 4 }}>Huruf besar/kecil tidak masalah, otomatis dirapikan & digabung dengan nama yang sama.</p>
                ...
                {!cekLoading && sisaInfo && sisaInfo.sisa !== null && (
                  <div style={{ marginTop: 6, padding: "8px 12px", background: sisaInfo.habis ? "#fff5f5" : sisaInfo.sisa <= 2 ? "#fffde7" : "#f1f8e9", border: `1px solid ${sisaInfo.habis ? "#ffcdd2" : sisaInfo.sisa <= 2 ? "#fff59d" : "#c5e1a5"}`, fontSize: "0.75rem", fontFamily: "var(--font-mono)" }}>
                    {sisaInfo.habis
                      ? <span style={{ color: "#c62828", fontWeight: 700 }}>⚠ Paket PT sudah habis ({sisaInfo.completed}/{sisaInfo.total} sesi)</span>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          ...
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", width: "100%", maxWidth: 400, padding: 28 }}>
            ...
            <p style={{ fontSize: "0.825rem", color: "#555", marginBottom: 20 }}>
              {justHabis
                ? <>Sesi terakhir paket <strong>{sesiForm.member || sisaInfo?.member}</strong> bersama pelatih <strong>{sesiForm.pelatih || sisaInfo?.pelatih}</strong> baru aja dicatat dan paketnya udah penuh. Mau langsung perpanjang?</>
                : <>Paket PT <strong>{sesiForm.member}</strong> bersama pelatih <strong>{sesiForm.pelatih}</strong> sudah selesai semua sesinya. Perpanjang dulu sebelum mencatat sesi baru.</>
              }
            </p>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontFamily: "var(--font-mono)", fontSize: "0.6rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#666", marginBottom: 8 }}>Pilih Paket Perpanjangan</label>
      ...
      {/* Modal Perpanjang dari Data Keseluruhan */}
      {perpanjangTarget && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          onClick={e => { if (e.target === e.currentTarget) setPerpanjangTarget(null); }}>
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", width: "100%", maxWidth: 400, padding: 28 }}>
            <div style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.1rem", color: "#1a1a1a", marginBottom: 4 }}>Perpanjang Paket PT</div>
            <p style={{ fontSize: "0.825rem", color: "#888", marginBottom: 20 }}>
              <strong>{perpanjangTarget.member}</strong> · Pelatih {perpanjangTarget.pelatih}
            </p>
            <label style={{ display: "block", fontFamily: "var(--font-mono)", fontSize: "0.6rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#666", marginBottom: 10 }}>Pilih Paket Baru</label>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 20 }}>
              {paketPTOptions.map(o => (
                <button key={o.value} onClick={() => setPerpanjangTargetPaket(o.value)}
                  style={{ padding: "10px 12px", border: `2px solid ${perpanjangTargetPaket === o.value ? "#1a1a1a" : "#e0e0e0"}`, background: perpanjangTargetPaket === o.value ? "#1a1a1a" : "#fff", color: perpanjangTargetPaket === o.value ? "#fff" : "#1a1a1a", fontFamily: "var(--font-body)", fontSize: "0.8rem", fontWeight: perpanjangTargetPaket === o.value ? 700 : 400, cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}>
                  {o.label}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              <button onClick={() => setPerpanjangTarget(null)} style={{ background: "none", border: "1px solid #ddd", color: "#888", fontFamily: "var(--font-body)", fontSize: "0.8rem", padding: "9px 18px", cursor: "pointer" }}>Batal</button>
              <button disabled={perpanjangTargetLoading} onClick={async () => {
                setPerpanjangTargetLoading(true);
                try {
                  await api.post("/pelatih/perpanjang", { pelatih: perpanjangTarget.pelatih, member: perpanjangTarget.member, paket: perpanjangTargetPaket });
                  setPerpanjangTarget(null);
                  fetchData();
                } catch (err) { alert("Gagal: " + err.message); }
                setPerpanjangTargetLoading(false);
              }} style={{ background: "#1565c0", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.8rem", padding: "9px 20px", cursor: perpanjangTargetLoading ? "default" : "pointer", opacity: perpanjangTargetLoading ? 0.6 : 1 }}>
                {perpanjangTargetLoading ? "Memproses..." : "Perpanjang"}
              </button>
            </div>
          </div>
        </div>
