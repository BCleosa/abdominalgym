...
function computeRingkasanBulanan(riwayatPelatih, gajiKaryawan) {
  ...
  return Object.values(map)
    .map(m => ({ ...m, total: m.gajiPelatih + m.gajiKaryawan }))
...
export default function GajiPelatihKaryawan() {
  ...
  const openBayar = (cycle) => {
    setBayarCycle(cycle);
    setBayarForm({ jumlah: cycle.gajiDisarankan ? String(cycle.gajiDisarankan) : "", metode: "Cash", keterangan: "" });
    setBayarError("");
  };

  const handleBayarSubmit = async (e) => {
    e.preventDefault();
    setBayarError(""); setBayarLoading(true);
    try {
  ...
  const handleExportRiwayatPelatih = () => {
    const rows = gajiPelatih.riwayatDibayar.map(c => ({ "Tanggal Bayar": c.tanggalBayar, "Pelatih": c.pelatih, "Member": c.member, "Paket": c.paket, "Milestone": c.milestoneLabel || "-", "Jumlah Dibayar": c.jumlah, "Metode": c.metode }));
    exportToExcel(rows, "Riwayat_Gaji_Pelatih", "Gaji Pelatih");
  };
  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e0e0e0", padding: 20, marginBottom: 24 }}>
        ...
        {ringkasanBulanan.length === 0 ? <p style={{ color: "#aaa", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Belum ada data gaji yang tercatat.</p> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.825rem" }}>
            <thead><tr>{["Bulan", "Gaji Pelatih", "Gaji Karyawan", "Total"].map(h => <th key={h} style={{ background: "#f5f5f5", color: "#666", fontFamily: "var(--font-mono)", fontSize: "0.65rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "8px 12px", textAlign: h === "Bulan" ? "left" : "right", borderBottom: "1px solid #1a1a1a" }}>{h}</th>)}</tr></thead>
            <tbody>{ringkasanBulanan.map(m => <tr key={m.bulan} style={{ borderBottom: "1px solid #f0f0f0" }}><td style={{ padding: "8px 12px", fontWeight: 600, color: "#1a1a1a" }}>{formatBulan(m.bulan)}</td><td style={{ padding: "8px 12px", textAlign: "right", fontFamily: "var(--font-mono)", color: "#555" }}>{fmt(m.gajiPelatih)}</td><td style={{ padding: "8px 12px", textAlign: "right", fontFamily: "var(--font-mono)", color: "#555" }}>{fmt(m.gajiKaryawan)}</td><td style={{ padding: "8px 12px", textAlign: "right", fontFamily: "var(--font-mono)", fontWeight: 700, color: "#c62828" }}>{fmt(m.total)}</td></tr>)}</tbody>
          </table>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #1e1e1e", marginBottom: 20 }}>
        {[["pelatih", `Gaji Pelatih${gajiPelatih.siapDibayar.length > 0 ? ` (${gajiPelatih.siapDibayar.length})` : ""}`], ["karyawan", "Gaji Karyawan"]].map(([id, label]) => (
          <button key={id} onClick={() => setActiveTab(id)} style={{ background: "none", border: "none", borderBottom: activeTab === id ? "2px solid #1a1a1a" : "2px solid transparent", padding: "9px 20px", marginBottom: -1, fontFamily: "var(--font-body)", fontWeight: activeTab === id ? 600 : 400, fontSize: "0.875rem", color: activeTab === id ? "#1a1a1a" : "#888", cursor: "pointer", transition: "all 0.2s" }}>{label}</button>
        ))}
      </div>

      {loading ? <p style={{ color: "#aaa", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Memuat...</p> : activeTab === "pelatih" ? (
        <div>
          <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem", color: "#1a1a1a", marginBottom: 4 }}>Siap Dibayar</div>
          <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "#999", marginBottom: 14 }}>Pembayaran gaji per setengah paket — 8x dibayar di sesi ke-4 dan ke-8, 12x di sesi ke-6 dan ke-12, dst.</p>
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", overflow: "auto", marginBottom: 28 }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.825rem" }}>
              <thead><tr>{["Pelatih", "Member", "Paket", "Milestone", "Progress", "Saran Gaji", "Aksi"].map(h => <th key={h} style={{ background: "#f5f5f5", color: "#666", fontFamily: "var(--font-mono)", fontSize: "0.65rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 14px", textAlign: "left", borderBottom: "1px solid #1a1a1a" }}>{h}</th>)}</tr></thead>
              <tbody>
                {gajiPelatih.siapDibayar.length === 0 ? <tr><td colSpan={7} style={{ padding: 28, textAlign: "center", color: "#aaa", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Belum ada milestone yang siap dibayar.</td></tr>
                : gajiPelatih.siapDibayar.map(c => (
                  <tr key={c.cycleId} style={{ borderBottom: "1px solid #f0f0f0" }}>
                    ...
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: "#f57f17", background: "#fff8e1", border: "1px solid #ffe082", padding: "2px 8px" }}>{c.milestoneLabel}</span></td>
                    ...
                    <td style={{ padding: "10px 14px" }}><button onClick={() => openBayar(c)} style={{ background: "#1a1a1a", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "0.7rem", padding: "6px 14px", cursor: "pointer" }}>Tandai Dibayar</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", overflow: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.825rem" }}>
              <thead><tr>{["Tanggal Bayar", "Pelatih", "Member", "Paket", "Milestone", "Jumlah", "Metode"].map(h => <th key={h} style={{ background: "#f5f5f5", color: "#666", fontFamily: "var(--font-mono)", fontSize: "0.65rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 14px", textAlign: "left", borderBottom: "1px solid #1a1a1a" }}>{h}</th>)}</tr></thead>
              <tbody>
                {gajiPelatih.riwayatDibayar.length === 0 ? <tr><td colSpan={7} style={{ padding: 28, textAlign: "center", color: "#aaa", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Belum ada riwayat pembayaran.</td></tr>
                : gajiPelatih.riwayatDibayar.map(c => (
                  <tr key={c.cycleId} style={{ borderBottom: "1px solid #f0f0f0" }}>
                    ...
                    <td style={{ padding: "10px 14px" }}><span style={{ fontFamily: "var(--font-mono)", fontSize: "0.62rem", color: "#2e7d32", background: "#e8f5e9", padding: "2px 8px" }}>{c.milestoneLabel || "-"}</span></td>
        ...
        <div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", overflow: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.825rem" }}>
              <thead><tr>{["Tanggal", "Keterangan", "Jumlah", "Metode"].map(h => <th key={h} style={{ background: "#f5f5f5", color: "#666", fontFamily: "var(--font-mono)", fontSize: "0.65rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "10px 14px", textAlign: "left", borderBottom: "1px solid #1a1a1a" }}>{h}</th>)}</tr></thead>
              <tbody>
                {gajiKaryawan.length === 0 ? <tr><td colSpan={4} style={{ padding: 28, textAlign: "center", color: "#aaa", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>Belum ada catatan gaji karyawan.</td></tr>
                : gajiKaryawan.map(g => <tr key={g.id} style={{ borderBottom: "1px solid #f0f0f0" }}><td style={{ padding: "10px 14px", color: "#666", fontFamily: "var(--font-mono)", fontSize: "0.75rem" }}>{g.tanggal}</td><td style={{ padding: "10px 14px", color: "#1a1a1a" }}>{g.keterangan}</td><td style={{ padding: "10px 14px", color: "#c62828", fontFamily: "var(--font-mono)", fontWeight: 600 }}>{fmt(g.jumlah)}</td><td style={{ padding: "10px 14px", color: "#888", fontSize: "0.8rem" }}>{g.metode}</td></tr>)}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {bayarCycle && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => { if (e.target === e.currentTarget) setBayarCycle(null); }}>
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", width: "100%", maxWidth: 420, padding: 28 }}>
            ...
            <p style={{ fontSize: "0.8rem", color: "#888", marginBottom: 4 }}>{bayarCycle.pelatih} — {bayarCycle.member} ({bayarCycle.paket})</p>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "#f57f17", marginBottom: 14 }}>Milestone: {bayarCycle.milestoneLabel}</p>
            {bayarCycle.hargaPaket > 0 && (
              <div style={{ background: "#fafafa", border: "1px solid #eee", padding: "10px 14px", marginBottom: 16, fontSize: "0.75rem", color: "#666", lineHeight: 1.8 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Harga porsi ini</span><span>{fmt(bayarCycle.hargaPaket)}</span></div>
            ...
            <form onSubmit={handleBayarSubmit}>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                ...
                <button type="submit" disabled={bayarLoading} style={{ background: "#1a1a1a", color: "#fff", border: "none", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.8rem", padding: "9px 20px", cursor: bayarLoading ? "default" : "pointer", opacity: bayarLoading ? 0.6 : 1 }}>{bayarLoading ? "Memproses..." : "Konfirmasi"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showKaryawanForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={e => { if (e.target === e.currentTarget) { setShowKaryawanForm(false); setAbsenInfo(null); } }}>
            ...
            <form onSubmit={handleKaryawanSubmit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                <div>
                  ...
                  <select style={{ ...inputStyle, appearance: "none" }} value={karyawanForm.namaKaryawan} onChange={e => { setKaryawanForm(p => ({ ...p, namaKaryawan: e.target.value })); fetchAbsen(e.target.value, karyawanForm.bulan); }} required>
                ...
                <div>
                  ...
                  <input type="month" style={inputStyle} value={karyawanForm.bulan} onChange={e => { setKaryawanForm(p => ({ ...p, bulan: e.target.value })); fetchAbsen(karyawanForm.namaKaryawan, e.target.value); }} onFocus={e => e.target.style.borderColor="#aaa"} onBlur={e => e.target.style.borderColor="#ddd"} />
                </div>
              </div>
              ...
              {!absenLoading && absenInfo && (
                <div style={{ background: "#f5f5f5", border: "1px solid #e0e0e0", padding: "12px 14px", marginBottom: 14, fontSize: "0.8rem" }}>
                  ...
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", color: "#555" }}>Auto: {absenInfo.hadir} hari × Rp {Number(absenInfo.gajiHarian).toLocaleString("id-ID")} = <strong style={{ color: "#1a1a1a" }}>Rp {Number(absenInfo.totalGaji).toLocaleString("id-ID")}</strong></div>
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                <div>
