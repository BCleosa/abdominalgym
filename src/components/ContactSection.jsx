...
export default function ContactSection() {
  const [form, setForm] = useState({ name: "", phone: "", membership: "", pt: "" });
  const [sent, setSent] = useState(false);

  const handleSubmit = (e) => {
    e.preventDefault();
    const paket = [form.membership, form.pt].filter(Boolean).join(" + ");
    const msg = `Halo Abdominal Gym! Nama saya ${form.name}, nomor HP ${form.phone}. Saya tertarik dengan ${paket || "informasi gym"}.`;
    window.open(`https://wa.me/6282324720045?text=${encodeURIComponent(msg)}`, "_blank");
    setSent(true);
    setTimeout(() => setSent(false), 3000);
    setForm({ name: "", phone: "", membership: "", pt: "" });
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
          ...
          <div style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", padding: "32px 28px" }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Membership */}
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>Membership (opsional)</label>
                <select value={form.membership} onChange={e => setForm({...form, membership: e.target.value})}
                  style={{ ...inputStyle, color: form.membership ? "#f0ede8" : "#555", appearance: "none" }}
                  onFocus={e => e.target.style.borderColor = "#444"}
                  onBlur={e => e.target.style.borderColor = "#222"}
                >
              ...
              <div style={{ marginBottom: 22 }}>
                <label style={labelStyle}>Personal Training <span style={{ color: "#444", fontWeight: 300, textTransform: "none", letterSpacing: 0 }}>— tidak wajib, bisa dikombinasikan dengan membership</span></label>
                <select value={form.pt} onChange={e => setForm({...form, pt: e.target.value})}
                  style={{ ...inputStyle, color: form.pt ? "#f0ede8" : "#555", appearance: "none" }}
                  onFocus={e => e.target.style.borderColor = "#444"}
                  onBlur={e => e.target.style.borderColor = "#222"}
                >
                  <option value="">Pilih paket PT...</option>
                  <option>1 Day Trial (Rp 150.000)</option>
                  <option>8× Coaching (Rp 600.000)</option>
                  <option>12× Coaching (Rp 800.000)</option>
                  <option>16× Coaching (Rp 1.000.000)</option>
                </select>
              </div>
              ...
              <button type="submit" style={{
                ...
                padding: "12px", border: "none", cursor: "pointer", transition: "opacity 0.2s",
              }}
